...
ARANCEL_ANUAL_COL = "ARANCEL ANUAL 2026"
ARANCEL_REFERENCIA_COL = "Arancel de Referencia 2026 Final Créditos (incremento = 2%)"
...
def _build_record(row: pd.Series, normalized_columns: dict[str, str], sheet_name: str) -> dict:
    ...
    for original_col, normalized_col in normalized_columns.items():
        if original_col in {ARANCEL_ANUAL_COL, ARANCEL_REFERENCIA_COL}:
            continue
        record[normalized_col] = _to_lower_strings(row.get(original_col))

    arancel_anual_monto = _parse_amount(row.get(ARANCEL_ANUAL_COL))
    arancel_referencia_creditos_monto = _parse_amount(row.get(ARANCEL_REFERENCIA_COL))
    ...
    record["arancel_de_referencia_con_creditos"] = {
        "2026": {
            "monto": arancel_referencia_creditos_monto,
        }
    }
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Lee aranceles de referencia 2026 creditos y convierte las filas de 3 hojas en objetos JSON."
    )
    parser.add_argument(
        "-i",
        "--input",
        default="aranceles_de_referencia_2026_creditos.xlsx",
        help="Ruta del archivo XLSX de entrada.",
    )
    parser.add_argument(
        "-o",
        "--output",
        default="resultado_json/aranceles_referencia_2026_creditos_objetos.json",
